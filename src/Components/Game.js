...
export default class Game extends Component {
  ...
  componentDidMount() {
    this.state.timer.on("render", this.moveSnake);
  ...
  changeInterval() {
    const currentFPSValue = this.state.timer.getFps();
    if (currentFPSValue < 30) {
      this.state.timer.setFps(currentFPSValue + 1);
    }
  }
  ...
  createNewMeal = () => {
    let newFoodPosition = [
      Math.floor(Math.random() * this.getColumnsNumber()),
      Math.floor(Math.random() * this.getRowsNumber())
    ];

    let foodOnSnake = this.state.snake.filter(
      elem => elem[0] === newFoodPosition[0] && elem[1] === newFoodPosition[1]
    );
  ...
  createSpecialMeal = () => {
    ...
    let foodOnSnake = this.state.snake.filter(
      elem =>
        elem[0] === newFlowerPosition[0] && elem[1] === newFlowerPosition[1]
    );
    if (foodOnSnake.length > 0) {
  ...
  moveSnake = () => {
    ...
    switch (Game.Model.lastKeyPressed) {
      ...
      case Game.Model.DIR_RIGHT:
        if (Game.Model.direction !== Game.Model.DIR_LEFT)
          Game.Model.direction = Game.Model.DIR_RIGHT;
        break;
      case Game.Model.DIR_DOWN:
        if (Game.Model.direction !== Game.Model.DIR_UP)
          Game.Model.direction = Game.Model.DIR_DOWN;
        break;
      case Game.Model.DIR_LEFT:
        if (Game.Model.direction !== Game.Model.DIR_RIGHT)
          Game.Model.direction = Game.Model.DIR_LEFT;
        break;
      case Game.Model.DIR_UP:
        if (Game.Model.direction !== Game.Model.DIR_DOWN)
          Game.Model.direction = Game.Model.DIR_UP;
        break;
    }
    ...
    if (isCollisionDetected && !this.props.settings.edgeWrapping) {
      this.gameOver();
      return;
    }
    ...
    if (
      headPosition[0] === this.state.food[0] &&
      headPosition[1] === this.state.food[1]
    ) {
    ...
    if (
      this.state.flower[2] === true &&
      headPosition[0] === this.state.flower[0] &&
      ...
      this.setState(prevState => {
        ...
        copyFlower[2] = false;
        this.state.timer.setFps(this.props.settings.speed);
